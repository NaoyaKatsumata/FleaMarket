        ...
        <div class="w-1/2">
            <div class="w-[80%] mx-auto">
                ...
                <div class="mt-8 border border-gray-400 h-[400px] overflow-auto">
                    ...
                    <form action="/comment" method="post" class="mt-16">
                        @csrf
                        @method('PUT')
                        <p>商品へのコメント</p>
                        <textarea class="w-full resize-none rounded" id="comment" name="comment" rows="3"></textarea>
                        <input type="hidden" name="userId" value="{{$userId}}">
                        <input type="hidden" name="itemId" value="{{$item->id}}">
                        <input type="submit" class="w-full mt-[20px] py-[10px] bg-red-500 text-white text-center rounded" name="submit" value="コメントを送信する">
                    </form>
            </div>
        </div>
